export function ExperienceContainer() {
    return (
        <div className="w-screen flex items-center justify-between mx-auto">
           
            <div className="w-96 space-y-10 ">
              <h2 className="text-zinc-100 font-bold text-4xl">Experience</h2>
              <p className="text-text-secondary text-2xl">
                3 years working as a Software Developer, in companys such as Itaú,
                Stone and Aftersale.
              </p>
            </div>

            <div className="w-96 space-y-10 ">
              <h2 className="text-zinc-100 font-bold text-4xl">Education</h2>
              <p className="text-text-secondary text-2xl ">
                Computer Science Bachelors Degree - Universidade Federal de Pelotas
                🎓
              </p>
            </div>

            <div className="w-96 space-y-10 ">
              <h2 className="text-zinc-100 font-bold text-4xl">Languages</h2>
              <p className="text-text-secondary text-2xl">🇺🇸 english</p>
              <p className="text-text-secondary text-2xl">🇧🇷 portugues</p>
            </div>
        </div>
    )
}